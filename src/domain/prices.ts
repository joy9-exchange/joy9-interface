...
export async function getLimitChartPricesFromStats(chainId, symbol, period, limit = 1) {
  ...
  const url = `${getServerUrlNew(chainId, '/candleprices')}?symbol=${symbol}&period=${period}&source=fast&from=${from}&chainId=${chainId}&limit=${limit}`;
...
export async function getChartPricesFromStats(chainId, symbol, period) {
  ...
  const url = `${getServerUrlNew(chainId, '/candleprices')}?symbol=${symbol}&period=${period}&from=${from}&source=fast&chainId=${chainId}`;
