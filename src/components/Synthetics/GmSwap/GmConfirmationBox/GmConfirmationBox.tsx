  ...
  const submitButtonState = (function getSubmitButtonState() {
    ...
    if (tokensToApprove.length > 0 && marketToken) {
      const symbols = tokensToApprove.map((address) => {
        return address === marketToken.address ? "JLP" : getTokenData(tokensData, address)!.symbol;
      });
    ...
    const text = t`Confirm ${operationText} ${formatTokenAmount(marketTokenAmount, marketToken?.decimals)} JLP`;
                ...
                {tokensToApprove.map((address) => (
                  <div key={address}>
                    <ApproveTokenButton
                      ...
                      tokenSymbol={address === marketToken?.address ? "JLP" : getToken(chainId, address).symbol}
                      spenderAddress={routerAddress}
                    />
                  </div>
                ))}
