...
export const MAX_TIMEOUT = 20000000;
...
export async function executeMulticall(
  ...
  const multicall = getMulticallLib(provider, chainId);
  ...
  const requestPromise = Promise.race([
    ...
    const multicall = getMulticallLib(fallbackProvider, chainId);
...
function getMulticallLib(provider: ethers.providers.Provider, chainId: number) {
  if (chainId === 31337) {
    return new Multicall({
      // @ts-ignore inconsistent provider types
      ethersProvider: provider,
      tryAggregate: false,
      multicallCustomContractAddress: "0x8A93d247134d91e0de6f96547cB0204e5BE8e5D8",
    });
  }
  ...
  if (chainId === 84531) {
    return new Multicall({
      ...
      ethersProvider: provider,
      tryAggregate: false,
      multicallCustomContractAddress: "0xE23b66e48790b33d209AF1E2879Ed77A65dCDa78",
    });
  }
