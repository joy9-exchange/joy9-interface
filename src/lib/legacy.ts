...
export function getBalanceAndSupplyData(balances) {
  ...
  //"glp", 
  const keys = ["gmx", "esGmx", "stakedGmxTracker"];
  const balanceData = {};
  const supplyData = {};
  const propsLength = 2;
...
export function getStakingData(stakingInfo) {
  ...
  //, "stakedGlpTracker", "feeGlpTracker"
  const keys = ["stakedGmxTracker", "bonusGmxTracker", "feeGmxTracker"];
  const data = {};
  const propsLength = 5;
...
export function getProcessedData(
  ...
  */
  ...
  data.stakedGlpTrackerRewards = bigNumberify(0);
  data.stakedGlpTrackerRewardsUsd = bigNumberify(0);
  ...
  data.glpVesterRewards = bigNumberify(0);
  ...
  data.feeGlpTrackerRewards = bigNumberify(0);
  data.feeGlpTrackerRewardsUsd = bigNumberify(0);
...
export function getPageTitle(data) {
  return `${data} | Decentralized Perpetual Exchange | JOY9`;
}
